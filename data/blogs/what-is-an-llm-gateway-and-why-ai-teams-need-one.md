...
[![Diagram showing applications connected to an Odock gateway before provider APIs, MCP tools, and plugins](/blog/llm-gateway-control-plane.svg "Odock centralizes provider routing, policy enforcement, observability, and tool access behind one stable endpoint.")](/blog/llm-gateway-control-plane.svg)

```mermaid
flowchart LR
  Apps["Product apps"] --> Gateway["Odock LLM gateway"]
  Gateway --> Policies["Policies and guardrails"]
  Gateway --> Usage["Budgets and observability"]
  Gateway --> Providers["LLM providers"]
  Gateway --> Tools["MCP tools and plugins"]
```
...
[![Schéma montrant des applications reliées à un gateway Odock avant les APIs provider, les outils MCP et les plugins](/blog/llm-gateway-control-plane.svg "Odock centralise le routing provider, l’application des politiques, l’observability et l’accès aux outils derrière un endpoint stable.")](/blog/llm-gateway-control-plane.svg)

```mermaid
flowchart LR
  Apps["Apps produit"] --> Gateway["Gateway LLM Odock"]
  Gateway --> Policies["Politiques et guardrails"]
  Gateway --> Usage["Budgets et observability"]
  Gateway --> Providers["Providers LLM"]
  Gateway --> Tools["Outils MCP et plugins"]
```
...
```mermaid
flowchart LR
  Apps["App prodotto"] --> Gateway["Gateway LLM Odock"]
  Gateway --> Policies["Policy e guardrails"]
  Gateway --> Usage["Budget e observability"]
  Gateway --> Providers["Provider LLM"]
  Gateway --> Tools["Tool MCP e plugin"]
```
